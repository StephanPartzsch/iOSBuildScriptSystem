...
def setup_parameters
	...
	unless build_directory_path.empty?; @build_directory_path = build_directory_path end
	puts "🔸  [BUILD_DIRECTORY_PATH] \t\t" + @build_directory_path + ""
		
	reports_directory_path = `echo "$REPORTS_DIRECTORY_PATH"`.strip
	unless reports_directory_path.empty?; @reports_directory_path = reports_directory_path end
	puts "🔸  [REPORTS_DIRECTORY_PATH] \t\t" + @reports_directory_path

	info_plist_path = `echo "$INFO_PLIST_PATH"`.strip
	unless info_plist_path.empty?; @info_plist_path = info_plist_path end
	puts "🔸  [INFO_PLIST_PATH] \t\t\t" + @info_plist_path + ""
	
	puts ""
	
	project_name = `echo "$PROJECT_NAME"`.strip
	unless project_name.empty?; @project_name = project_name end
	puts "🔸  [PROJECT_NAME] \t\t\t" + @project_name + ""
	
	scheme = `echo "$SCHEME"`.strip
	unless scheme.empty?; @scheme = scheme end
	puts "🔸  [SCHEME] \t\t\t\t" + @scheme + ""
	
	configuration = `echo "$CONFIGURATION"`.strip
	unless configuration.empty?; @configuration = configuration end
	puts "🔸  [CONFIGURATION] \t\t\t" + @configuration + ""
	
	puts ""
	
	file_name = `echo "$FILE_NAME"`.strip
	unless file_name.empty?; @file_name = file_name end
	puts "🔸  [FILE_NAME] \t\t\t\t" + @file_name + ""
	
	append_version_to_file_name = `echo "$APPEND_VERSION_TO_FILE_NAME"`.strip
	unless append_version_to_file_name.empty?; @append_version_to_file_name = append_version_to_file_name end
	...
	if @append_version_to_file_name
		...
		unless version_prefix.empty?; @version_prefix = version_prefix end
		puts "🔸  [VERSION_PREFIX] \t\t\t" + @version_prefix + ""
		
		puts "🔹  App version \t\t\t\t" + @app_version + ""
		
		version_suffix = `echo "$VERSION_SUFFIX"`.strip
		unless version_suffix.empty?; @version_suffix = version_suffix end
	...
	if @bundle_identifier
		...
		unless bundle_identifier.empty?; @bundle_identifier = bundle_identifier end
		puts "🔸  [BUNDLE_IDENTIFIER] \t\t\t" + @bundle_identifier + ""
		puts ""
	end
	
	source_directories = `echo "$SOURCE_DIRECTORIES"`.strip
	unless source_directories.empty?; @source_directories = source_directories end
	...
	if @user_defined_arguments
		...
		unless user_defined_arguments.empty?; @user_defined_arguments = user_defined_arguments end
	...
	if @preprocessor_macros
		...
		unless preprocessor_macros.empty?; @preprocessor_macros = preprocessor_macros end
